...
class HomographyTestSuite(unittest.TestCase):
    ...
    def test_computeHomography(self):
        ...
        with np.load("TestImages/Variables.npz") as npFile:
            expectedValue = np.round(npFile["homography"], 15)

        h = Homography(sourcePoints = sourcePoints, targetPoints = targetPoints)
        actualValue = np.round(h.forwardMatrix, 15)

        isMatched = np.all(expectedValue == actualValue)
    ...
    def test_transformImageGray(self):
        ...
        with self.subTest(key = "Gray No Effect"):
            ...
            containerImage = imread("TestImages/WhiteGray.png")
            actualValue = transform.transformImageOnto(containerImage)
            expectedValue = imread("TestImages/Target_knight.png")
        ...
        for effect in Effect:
            with self.subTest(key = "Gray " + effect.name):
                ...
                containerImage = imread("TestImages/WhiteGray.png")
                actualValue = transform.transformImageOnto(containerImage)
                expectedValue = imread("TestImages/Target_knight_{}.png".format(effect.name))
    ...
    def test_transformImageColor(self):
        ...
        """
        sourceImage = imread("TestImages/strange.png")

        with self.subTest(key = "Color No Effect"):
            transform = ColorTransformation(sourceImage)
            transform.setupTransformation(self.targetPoints)

            containerImage = imread("TestImages/White.png")
            actualValue = transform.transformImageOnto(containerImage)
            expectedValue = imread("TestImages/Target_strange.png")
        ...
        for effect in Effect:
            with self.subTest(key = "Gray " + effect.name):
                ...
                containerImage = imread("TestImages/White.png")
                actualValue = transform.transformImageOnto(containerImage)
                expectedValue = imread("TestImages/Target_strange_{}.png".format(effect.name))
...
class MoreHomographyTestSuite(unittest.TestCase):
    ...
    def test_applyEffectV(self):
        ...
        """
        sourceImagePath = "TestImages/Ring.png"
        ...
        for v, h1, h2 in self.parameterList:
            with self.subTest(key=(v, h1, h2)):
                ...
                resultImagePath = "TestImages/Ring_EffectV_{0:03d}_{1:03d}_{2:03d}.png".format(v, h1, h2)
    ...
    def test_applyEffectA(self):
        ...
        """
        sourceImagePath = "TestImages/Ring3.png"
        sourceImage = imread(sourceImagePath)
        ...
        for v, h1, h2 in self.parameterList:
            with self.subTest(key=(v, h1, h2)):
                ...
                resultImagePath = "TestImages/Ring_EffectA_{0:03d}_{1:03d}_{2:03d}.png".format(v, h1, h2)
                expectedValue = imread(resultImagePath)
